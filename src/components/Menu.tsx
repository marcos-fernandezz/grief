...
export default function Menu() {
  ...
  // 👇 NUEVO ESTADO: Controla el menú lateral en celular
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Calculamos la altura total de la pantalla del usuario
      const screenHeight = window.innerHeight;

      // Cambiamos a blanco justo unos 60px antes de cruzar la línea 
      // (que es más o menos lo que mide de alto tu menú)
    ...
    // Lo ejecutamos una vez al cargar por si el usuario recarga la página a la mitad
    handleScroll();

    return () => window.removeEventListener("scroll", handleScroll);
  }, []);
  // Lógica del Debounce para el buscador (Queda intacta)
  useEffect(() => {
  ...
  const navStyle = isHome && !isScrolled
    ? "fixed w-full z-40 bg-transparent text-white py-4 md:py-6" // Transparente arriba
    : "fixed w-full z-40 bg-white border-b border-neutral-100 text-black py-4"; // Blanco al bajar o en otras páginas
  return (
    <>
      <nav className={`transition-all duration-300 ${navStyle}`}>
        {/* 👇 MOBILE FIRST: Flex en celu, Grid en escritorio 👇 */}
        <div className="flex justify-between md:grid md:grid-cols-3 items-center w-full max-w-[1600px] mx-auto px-6 md:px-12 lg:px-16 relative h-12 md:h-auto">
          ...
          {/* 💻 DESKTOP: NAVEGACIÓN Y MEGA MENÚ (Oculto en celular) */}
          ...
          {/* 👇 CENTRO: LOGO (Absoluto en celu para que no se corra) 👇 */}
          ...
          <div className="flex justify-end items-center gap-3 md:gap-5 text-[10px] uppercase tracking-widest font-bold">
            ...
            {/* Ocultamos el User en celu para ahorrar espacio */}
            ...
            <div className={`hidden md:block h-4 w-[1px] ${isHome ? 'bg-white/30' : 'bg-neutral-200'}`}></div>
      ...
      <div className={`fixed inset-0 bg-black/60 z-50 transition-opacity duration-300 md:hidden ${isMobileMenuOpen ? "opacity-100 visible" : "opacity-0 invisible"}`}>
        <div className={`fixed top-0 left-0 w-[85%] max-w-sm h-full bg-white text-black p-6 transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"}`}>
          ...
          <div className="flex flex-col gap-6 text-sm font-bold tracking-widest uppercase">
            <Link href="/tienda" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-neutral-500">Tienda</Link>
            <Link href="/tienda?categoria=remeras" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-neutral-500">Remeras & Tops</Link>
            <Link href="/tienda?categoria=buzos" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-neutral-500">Buzos & Hoodies</Link>
            <Link href="/manifiesto" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-neutral-500">Manifiesto</Link>
            ...
            {status === "authenticated" ? (
              <>
                {session?.user && (session.user as any).role === "ADMIN" && (
                  <Link href="/admin" onClick={() => setIsMobileMenuOpen(false)} className="text-neutral-500">Panel Admin</Link>
                )}
                <button
                  onClick={() => { setIsMobileMenuOpen(false); clearCart(); signOut({ callbackUrl: '/login' }); }}
              ...
              <Link href="/login" onClick={() => setIsMobileMenuOpen(false)} className="text-neutral-500">Ingresar a mi cuenta</Link>
            )}
          </div>
        </div>

        {/* Fondo oscuro clickeable para cerrar el menú */}
        <div className="absolute inset-0 z-[-1]" onClick={() => setIsMobileMenuOpen(false)}></div>
      </div>

      {/* OVERLAY DEL BUSCADOR (Responsivo por defecto) */}
      {isSearchOpen && (
        <div className="fixed top-0 left-0 w-full h-20 md:h-24 bg-white text-black flex items-center justify-center z-50 animate-in slide-in-from-top-full duration-300 border-b border-neutral-100 shadow-sm">
          <div className="max-w-4xl w-full px-8 flex items-center gap-4">
            {/* 👇 SI ESTÁ PENSANDO, LA LUPA TITILA 👇 */}
            <Search className={`w-6 h-6 text-neutral-400 ${isPending ? "animate-pulse opacity-50" : ""}`} strokeWidth={1.5} />

            <input
              type="text"
              placeholder="BUSCAR..."
              className="w-full text-base md:text-xl font-light tracking-widest outline-none uppercase placeholder:text-neutral-300 bg-transparent"
              autoFocus
              value={searchTerm}
            ...
            <button
              onClick={() => {
                setIsSearchOpen(false); // 1. Cierra el panel visual
                setSearchTerm("");      // 2. Limpia el estado interno
                router.push("/");       // 3. 🚀 TELETRANSPORTACIÓN INMEDIATA A LA HOME
              }}
              className="p-2 hover:bg-neutral-100 rounded-full transition-colors -mr-2 md:mr-0"
            >
              <X className="w-5 h-5 md:w-6 md:h-6 text-neutral-400 hover:text-black" strokeWidth={1.5} />
            </button>
          </div>
        </div >
      )
